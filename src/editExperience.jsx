...
const AddExperience = () => {
  ...
  const loadExperience = async () => {
    const req = await fetch("/api/v2/experiences.php?id=" + id);
  ...
  const handleSubmit = async (evt) => {
    ...
    if (validateInputs()) {
      let jsonData = { ...data, id };
      const req = await fetch("/api/v2/editExperience.php", {
        method: "POST",
        headers: {
      ...
      if (res.success) {
        ...
        // Storage remove it
        navigate("/");
        userCtx.notify("Experience has been updated successfully", "success");
      }
    }
    setLoading(false);
  };
